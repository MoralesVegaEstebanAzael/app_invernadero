...
class TestApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyFirebaseApp(title: 'Phone Authentication'),
    );
  }
}

class MyFirebaseApp extends StatefulWidget {
  MyFirebaseApp({Key key, this.title}) : super(key: key);
...
class _MyFirebaseAppState extends State<MyFirebaseApp> {
  ...
  Future<void> _submitPhoneNumber() async {
    print("entra a submit phone number");
    /// NOTE: Either append your phone number country code or add in the code itself
    /// Since I'm in India we use "+91 " as prefix `phoneNumber`
    String phoneNumber = "+52 " + _phoneNumberController.text.toString().trim();
    print(phoneNumber);

    /// The below functions are the callbacks, separated so as to make code more redable
    void verificationCompleted(AuthCredential phoneAuthCredential) {
      print("verificacion chida");
    ...
    void codeAutoRetrievalTimeout(String verificationId) {
      ...
      //aqui recupera el codigo y se logea
      print(verificationId);
